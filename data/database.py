...
class Database:
    ...
    def create_database(self, database_name):
        ...
        self.db.setDatabaseName(os.path.dirname(
           os.path.realpath(__file__)) + '/' + database_name)
        ...
        query.exec_("CREATE TABLE movies(id INTEGER PRIMARY KEY AUTOINCREMENT,"
                    ...
                    " description varchar(20),duration INT ,"
                    " actors varchar(100),genre varchar(20),"
                    " icon_path varchar(100))")

        query.exec_("CREATE TABLE users(id INTEGER PRIMARY KEY AUTOINCREMENT,"
                    " login varchar(20),email varchar(20),"
                    " password varchar(20),icon_path varchar(100))")

        query.exec_("CREATE TABLE reviews("
                    " id INTEGER PRIMARY KEY AUTOINCREMENT,"
                    " player_id INTEGER,movie_id INTEGER, score INTEGER)")
    ...
    def get_field_by_parameter(self, table_name, field_name, parameter, value):
        ...
        """
        query = QtSql.QSqlQuery()
        query.exec_(f"SELECT {field_name}"
                    f" FROM {table_name} WHERE {parameter} = '{value}'")
        query.next()
    ...
    def register_user(self, login, password, email):
       ...
       return User object if succeeds
        ...
        if (len(email) == 0 or len(login) == 0 or len(password) == 0
           or self.get_field_by_parameter(
              "users", "login", "login", login) is not None
           or self.get_field_by_parameter(
              "users", "email", "email", email) is not None
           or email.find('@') <= 0):
            return None
    ...
    def login_user(self, login, password):
       ...
       return User object if succeeds
        ...
        return User(login, record.value("email"),
                    record.value("icon_path"), password, record.value("id"))
       ...
       Get all users from database
        ...
        while query.next():
            ...
            users.append(User(record.value("login"), record.value("email"),
                         record.value("password"), record.value("icon_path"),
                         record.value("id")))
    ...
    def get_movies_by_parameter(self, parameter, value):
       ...
       Get movies if they pass the specified parameter
        ...
        if parameter != "actors":
            query.exec_(f"SELECT * FROM movies WHERE"
                        f" {parameter} LIKE '{value}%'")
        else:
            query.exec_(f"SELECT * FROM movies WHERE"
                        f" {parameter} LIKE '%{value}%'")
        movies = []
    ...
    def get_user_reviews(self, user):
       ...
       Get reviews commited by specified user
        ...
        while query.next():
            record = query.record()
            reviews.append(Review(record.value("player_id"),
                           record.value("movie_id"),
                           record.value("score")))
    ...
    def add_movie(self, movie):
       ...
       return False if fails
        ...
        if (self.get_field_by_parameter(
           "movies", "title", "title", movie.get_title()) is not None
           or len(movie.get_title()) == 0 or len(movie.get_director()) == 0):
            return False

        query.exec_(f"""INSERT INTO movies
                    (title,director,description,duration,
                    actors,genre,icon_path)
                     VALUES ('{movie.get_title()}', '{movie.get_director()}'
                    ,'{movie.get_description()}',{movie.get_duration()},
                    '{movie.get_actors()}','{movie.get_genre()}',
                    '{movie.get_icon_path()}')""")
    ...
    def add_review(self, user_id, movie_id, score):
       ...
       return False if fails
        """
        if (self.get_field_by_parameter(
           "users", "id", "id", user_id) is None or
           self.get_field_by_parameter(
              "movies", "id", "id", movie_id) is None or
           score > 10 or score < 1):
            return False

        query = QtSql.QSqlQuery()

        query.exec_(f"""SELECT * FROM reviews WHERE player_id = {user_id}
                     AND movie_id = {movie_id}""")

        query.next()

        if query.isValid() is True:
            query.exec_(f"""UPDATE reviews SET score = {score} WHERE
                         player_id = {user_id} AND movie_id = {movie_id}""")
        else:
            query.exec_(f"""INSERT INTO reviews (player_id,movie_id,score)
