from PyQt5.QtWidgets import QLabel, QScrollArea, QGroupBox, QFormLayout
...
class MainWindow(FilmwebWindow):
    ...
    recommended movies , users , search bar for movies
    ...
    def create_profile_interface(self):
        ...
        self.profile_button.clicked.connect(
            lambda: self.show_profile(AppInstance.current_user))
    ...
    def show_movies(self, **kwargs):
        ...
        if "key" in kwargs and "value" in kwargs:
            movies = AppInstance.db.get_movies_by_parameter(
                self.search_key, self.search_line.text().strip())
        elif "sort" in kwargs:
            movies = sorted(AppInstance.db.get_movies(),
                            key=lambda movie: movie.get_avg_rate(),
                            reverse=True)
        else:
        ...
        for movie in movies:
            ...
            button.clicked.connect(
                lambda state, x=movie,
                label=avg_rate_label: self.rate_movie(x, label))
            details_button = QPushButton("&Details", self)
            details_button.clicked.connect(
                lambda state,
                x=movie: DetailsDialog.get_movie_details(x, self))
    ...
    def set_search_key(self, key):
        """
        Sets search key for search bar
        """
        self.search_key = key
    ...
    def search(self):
        ...
        if self.search_line.text().strip() != "":
            self.show_movies(
                key=self.search_key, value=self.search_line.text().strip())
        else:
            self.show_movies()
    ...
    def show_users(self):
        ...
        for user in AppInstance.db.get_users():
            ...
            button.clicked.connect(
                lambda state, x=user: self.show_profile(x))
    ...
    def rate_movie(self, movie, label):
        if AppInstance.current_user:
            ...
            QMessageBox.warning(self, 'Error',
                                'You need to be logged to rate a movie',
                                QMessageBox.Ok)
    ...
    def login(self):
        ...
        QMessageBox.information(self,
                                'Information:', 'Logged succesfully!',
                                QMessageBox.Ok)
    ...
    def register(self):
        ...
        if not login or not password or not email:
            QMessageBox.warning(self, 'Error',
                                'Empty login or email or password!',
                                QMessageBox.Ok)
            return
        ...
        if user is None:
            QMessageBox.warning(self, 'Error',
                                'Invalid login or email or password!',
                                QMessageBox.Ok)
            return
        ...
        QMessageBox.information(self,
                                'Information', 'Registered succesfully !',
                                QMessageBox.Ok)
